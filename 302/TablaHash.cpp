...
void TablaHash::reestructurar(){
    ...
    for (int i = 0; i < antiguaM; i++)
    {
        ...
        {
            h = funcionDispersion(itPar->getUsuario()); // Calculamos la nueva h
            nueva[h].push_back(move(*itPar));         // Usamos move para mover el par a la tabla nueva, de esta manera no lo copiamos por valor
            itPar++;     
        }
    }
...
Cuac *TablaHash::insertar (Cuac nuevo){
    ...
    Cuac *cuac_devolver;                            // Referencia del cuac insertado para devolverlo
        ...
        if (nuevo_usuario == par_usuario.getUsuario()){                 // Comparamos el usuario del cuac a insertar con el del par
            ...
            while (itCuac != lista_cuacs.end() && insertado == false){  // Recorreremos la lista de cuacs hasta llegar al final o insertarlo
                if (nuevo.es_anterior(*itCuac))  {                      // Vamos comprobando si el nuevo es anterior (más reciente) que el del par a comparar
                    // Si es así, se inserta y guardamos la referencia del Cuac insertado para devolverla luego
                    cuac_devolver = &(*lista_cuacs.insert(itCuac, nuevo));   // Contenido del it (*) y referencia (&)
            ...
            if (!insertado){
                ...
                cuac_devolver = &lista_cuacs.back();
                insertado = true;
            }
        }
    ...
    if (!insertado){
        ...
        cuac_devolver = &(T[h].back().getCuacs().back());   // Lo tenemos que coger de aquí, que es el que está en la tabla
    }
    nElem++;        // + 1 cuac nuevo
    // return &nuevo;   // Esto devolvería basura (variable local)!!! Hay que devolver la referencia al propio objeto dentro de la tabla
    return cuac_devolver; // Devolvemos la referencia al cuac insertado
}
